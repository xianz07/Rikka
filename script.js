// --- script.js (最终版，统一使用 verifyAndUseCode) ---
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function activateDevice() {
        ...
        try {
            const deviceId = getDeviceId();
            console.log(`准备调用云函数 'verifyAndUseCode'，参数: code=${codeInput}, deviceId=${deviceId}`);
            ...
            if (error.message && error.message.includes('404')) {
                 activationStatus.textContent = '激活失败：云函数未找到。请确认后台函数名是否正确。';
            } else {
    ...
    function initializeEventListeners() {
        ...
        addFieldBtn.addEventListener('click', () => {
             ...
             const label = prompt("请输入新字段的中文名 (如 'QQ号'):");
             if (key && label) {
    ...
    async function openDetailsModal(nodeId) {
        ...
        const knownKeys = new Set(PRESET_FIELDS.map(f => f.key));
        const systemKeys = new Set(['objectId', 'createdAt', 'updatedAt', 'ACL', 'label', 'bio', 'image']);

        allKeys.forEach(key => {
            if (systemKeys.has(key)) return; // 过滤掉系统字段和我们已在别处处理的字段

            const preset = PRESET_FIELDS.find(f => f.key === key);
            const label = preset ? preset.label : key; // 如果是自定义字段，用 key 作为 label
